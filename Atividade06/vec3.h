...
public:
    ...
    /**
     * @brief Escreve a cor no formato PPM (Red Green Blue).
     * 
     * @param out O stream de saída.
     * @param samples O número de amostras por pixel.
     */
    void write_color(std::ostream &out, int samples) const {
        // Calcula a média da cor e aplica a correção gama 2.
        auto scale = 1.0 / samples;
        auto r = sqrt(e[0] * scale);
        auto g = sqrt(e[1] * scale);
        auto b = sqrt(e[2] * scale);

        // Escreve os valores no formato PPM.
